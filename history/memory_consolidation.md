...
- Beacon anchoring: always attach __user_self__ and __agent_self__; full seven-beacon anchoring is aspirational.
...
- card_events mandatory: log merge, contradiction, prune with before/after payloads for audit and rollback; splits stay planned.
...
- Multi-faceted self beacons: during entity extraction, inject self-pattern beacons when strong signals appear: __self_identity__, __self_location__, __self_work__, __self_health__, __self_relationships__, __self_preferences__, __self_projects__.
...
Data Model
- consolidated_cards: add access_count, merge_version, beacon_list (JSON of beacon ids).
- beacon_registry: id, strength, card_count, generation, parent_beacon, last_touched_ms.
- card_events: append-only permanent log with card_id, event_type, payload_json, timestamp_ms, source_turn_id, before/after snapshots.

Processing and Wiring
- Store and consolidate turn: build combined text and embedding; extract and canonicalize entities; fingerprint; insert raw_turn. Candidates: entity overlap first, then ANN index. Apply adaptive threshold. Paths: fingerprint shortcut, duplicate, merge, contradiction reconcile, or new card. Update embeddings per hybrid strategy, bump access_count, emit card_events for every mutation.
...
Outcome
- A local, auditable cognitive layer: anchored by primordial beacons, expanded by discovered topics, contradiction-aware, fast at scale, and traceable via card_events.
