...
- Embeddings: default strategy blends existing card embedding with the new turn (new_weight 0.4); re-embed is used for contradictions or when configured.
...
- Retrieval: picks beacons from the query’s canonical entities (top-2 by strength/card_count if they exist) with primordial anchors as fallback; then tries beacon overlap, keyword overlap, and ANN similarity in that order.
...
- Hybrid embedding strategy: default is blend with new_weight 0.4 for normal merges; contradiction or correction paths re-embed reconciled summary. (Re-embed-only remains available as a strategy.)
...
- Phase 1 — Emergent Beacon Discovery (every 20 turns by default, configurable): co-occurrence clustering (PMI or Jaccard) over consolidated cards. When 4+ entities co-occur in 8+ cards with strength >= 0.75, promote synthetic beacon __beacon_{hash}__ (e.g., __topic_common_lisp__, __project_quantum_os__, __person_elon_musk__, __concept_category_theory__).
- Phase 2 — Beacon Lifecycle and Strength: beacon_registry tracks beacon_id, strength (0–1), card_count, generation, parent_beacon, last_touched_ms. Demote (strip from cards, lower strength, reset card_count) when strength < 0.5 and untouched 180 days; immortalize when strength > 0.98 and card_count > 100.
- Phase 3 — Hierarchical Retrieval: retrieve_context extracts query canonical entities, maps them to matching beacons (top-2 by strength/card_count), appends primordial fallback, and returns cards by beacon overlap, else keyword overlap on canonical entities, else ANN similarity plus recency slice.
...
- Summaries and titles: after each non-trivial turn, the orchestrator calls store_and_consolidate_turn with helper LLMs. New cards get a summary from the combined turn text via a concise summarizer and a short title generator; merges use a merge-updater, and contradictions use a reconciliation summarizer. Helper/chat models are configurable (GROQ_HELPER_MODEL defaulting to openai/gpt-oss-20b; GROQ_MODEL defaulting to openai/gpt-oss-120b); providers follow the config/env wiring. Summaries are blended by default; re-embed runs when configured or when contradiction reconciliation demands it.
- Beacon assignment (LLM, structured, hard-capped): during consolidation, after summary generation, a dedicated helper (Groq structured output) receives the summary, canonical entities, current card beacons (for merges), and the entire beacon_registry (primordials + all added beacons). The helper is expected to return at most one existing registry id or at most one new id (mutually exclusive) via JSON schema {existing: [id?], proposed_new: [id?]}; if it returns nothing, the system falls back to attaching any canonical entities already present in the registry. Any new id is inserted immediately into beacon_registry (generation=1, strength seed=0.6) and attached to the card. Self beacons from canonicalization are forced into the beacon list before this helper runs; primordial anchors are added separately after the helper selection.
